...
mutable struct NoiseVector{T,N,Tt,T2,T3,ZType,inplace} <: AbstractNoiseProcess{T,N,Vector{T2},inplace}
    t::Vector{Tt}
    u::Vector{T2}
    W::Vector{T2}
    Z::ZType
...
function NoiseVector(t,W,Z=nothing;reset=true,linear=false)
  ...
  NoiseVector{typeof(val),ndims(val),typeof(dt),typeof(dW),typeof(dZ),typeof(Z),iip}(
            t,W,W,Z,curt,curW,curZ,dt,dW,dZ,true,reset,linear)
end
...
function interpolate!(W::NoiseVector,t)
  ...
  sign(W.dt)*t > sign(W.dt)*ts[end] && error("Solution interpolation cannot extrapolate past the final timepoint. Build a longer NoiseVector to cover the integration.")
  sign(W.dt)*t < sign(W.dt)*ts[1] && error("Solution interpolation cannot extrapolate before the first timepoint. Build a longer NoiseVector to cover the integration.")
  ...
  isa(vals2,Nothing) ? val1 : (val1,val2)
end

function interpolate!(out1,out2,W::NoiseVector,t)
  ts,timeseries,timeseries2 = W.t,W.W,W.Z
  sign(W.dt)*t > sign(W.dt)*(ts[end]+10*sign(W.dt)*eps(typeof(t))) && error("Solution interpolation cannot extrapolate past the final timepoint. Build a longer NoiseVector to cover the integration.")
  sign(W.dt)*t < sign(W.dt)*(ts[1]-10*sign(W.dt)*eps(typeof(t))) && error("Solution interpolation cannot extrapolate before the first timepoint. Build a longer NoiseVector to cover the integration.")
  tdir = sign(ts[end]-ts[1])
