...
f0 = 2.0e3
α = 1
PinkNoise = GaussianNoiseProcess.GaussianProcess(f -> color_spectrum(f, f0, α, dt*tscale), dt, tscale; window_length=2^10, coeff_length=2^6)
...
H(J, Δ, κ, ν) = 2π * (J/2) * sigmax(sb) ⊗ one(vb) + 2π * (Δ/2) * sigmaz(sb) ⊗ one(vb) + 2π * (κ/2) * sigmaz(sb) ⊗ (create(vb) + destroy(vb)) + 2π * ν * one(sb) ⊗ number(vb)
...
sol = map(trial ->
stochastic.schroedinger(tspan, ψi, H(params.J, params.Δ, params.κ, ν), J_Δ(Γ); fout=fout, noise=PinkNoise(0,0), dt=dt, adaptive=adaptive, alg=alg, reltol=1e-8, abstol=1e-8, normalize_state=true)[2]
, 1:Ntrials) |> vec -> hcat(vec...) |> mat -> sum(mat,dims=2) ./ Ntrials
