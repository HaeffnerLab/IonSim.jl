...
wold_psd(sigma, psi, freq, T) = arma_psd(sigma, [0.0], -psi[2:end], freq, T; theta0=-psi[1])
arma_acf(sigma, psi) =  sigma^2 .* [dot(psi[1:end-i], psi[1+i:end]) for i in 0:(length(psi)-1)]
...
function estimate_IIR_coefficients(acf, psi_guess, coeff_length=length(psi_guess))
  ...
  psi, _ = so.curve_fit((x,d...)->WoldDecompACF(collect(d)), ts, ys, p0, jac=(x,d...)->WoldDecompACF_Jac(x,collect(d)))
  psi ./= psi[1]
...
@inline function YuleWalker!(phi, ρ; ρtrue=ρ)
  ...
  lsqr!(phi,Rmat,ρtrue[2:end], atol=1e-6, btol=1e-6, maxiter=max(maximum(size(Rmat)),16)) #IterativeSolvers
end
  ...
  if !isa(rtol, Nothing)
    ...
    @inbounds for k in 1:coeff_length
      ...
      if (k > 1) && isapprox(pacf[k], pacf[k-1], atol=100eps(), rtol=rtol)
        ...
        if abs(pacf[k]) < (1/sqrt(length(acf))) break end
      end
      push!(phi,0.0); push!(pacf,0.0)
    end
...
@inline function ARMA(psd::AbstractVector, freq::Union{AbstractVector,Frequencies}, dt::Real=1.0, timescale::Real=1.0; mean::Real=0.0, normalize_sigma::Bool=true,
  coeff_length::Union{Integer,Nothing}=nothing, impulse_length::Union{Integer,Nothing}=nothing, rtol=1e-6)
  ...
  (phi, ar_roots), theta, psi = generate_ARMA_coefficients(acf, coeff_length, impulse_length; rtol=rtol)
  ...
  return ARMA(freq, psd, ar_ps, ma_ps, acf, ar_acf, ma_acf, phi, theta, psi, ar_roots, mean, ar_sigma, ma_sigma, dt, timescale, normalize_sigma, coeff_length, impulse_length, length(freq), abs(freq[2] - freq[1]), rtol)
end
