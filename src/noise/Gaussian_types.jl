...
mutable struct ARMAProcess
    ...
    @inline function ARMAProcess(
        psd::Function,
        dt::Real,
        timescale::Real = 1.0;
        mean::Real = 0.0,
        normalize_sigma::Bool = true,
        Hfreq::Union{Function, Nothing} = nothing,
        Htime::Union{Function, Nothing} = nothing,
        window_length::Integer = 2^10,
        coeff_length::Union{Integer, Nothing} = 2^6,
        ...
        return new(arma, powerspectrum, BandwidthFilter, NoiseFilter, float(dt), timescale)
    end
end

@inline function (X::ARMAProcess)(
    t0::T,
    W0::T = 0.0,
    Z0 = nothing;
    response = "AR",
    ...
    if response == "AR"
        dist = AR_Process(
            ...
            arma.sigma,
            arma.mean,
            arma.mean * (1 - sum(arma.phi)),
            X.arma.dt,
            float(t0) + X.arma.dt,
            X.Htime,
            zeros(length(arma.phi))
        )
        for i in 1:min(256, 2 * dist.p)
            mean = dist.c + dot(dist.phi, dist.past)
            pushfirst!(dist.past, 0.0)
            pop!(dist.past)
    ...
    return NoiseProcess{false}(
        ...
        Z0,
        dist,
        (dW, W, W0, Wh, q, h, u, p, t, rng) ->
            bridge(dist, dW, W, W0, Wh, q, h, u, p, t, rng);
        rswm = RSWM(adaptivealg = :RSwM3),
        kwargs...
    )
end
...
struct CARMAProcess{T1, T2, T3, T4, ARMA, T5}
    ...
    @inline function CARMAProcess(
        arma::ARMA;
        Htime::Function = x -> false,
        fit_args::Union{Dict, Nothing} = nothing
    )
        ...
        if any(abs.(arma.phi) .> eps())
            ...
            carma_α, carma_sigma, carma_psd, carma_acf = ARMAtoCARMASolver(
                arma.dt,
                arma.acf,
                arma.phi,
                arma.psi,
                carma_λ,
                max(arma.sigma, arma.ma_sigma);
                fit_args = fit_args
            )
...
CARMAProcess(arma::ARMAProcess; kwargs...) =
    CARMAProcess(arma.arma; Htime = arma.Htime, kwargs...)
...
@inline function (X::CARMAProcess)(t0::T, W0::T, Z0 = nothing; kwargs...) where {T <: Real}
    ...
    return NoiseProcess{false}(
        ...
        dist,
        (dW, W, W0, Wh, q, h, u, p, t, rng) ->
            CARMABridge(dist, dW, W, W0, Wh, q, h, u, p, t, rng);
        rswm = RSWM(adaptivealg = :RSwM2),
        kwargs...
    )
end
...
mutable struct GaussianProcess
    ...
    @inline function GaussianProcess(
        psd::Function,
        dt::Real,
        timescale::Real = 1.0;
        mean::Real = 0.0,
        ...
        Htime::Union{Function, Nothing} = nothing,
        window_length::Integer = 2^10,
        coeff_length::Union{Integer, Nothing} = 2^7,
        impulse_length::Union{Integer, Nothing} = nothing,
        rtol::Real = 1e-6,
        ...
        # == Return ARMAProcess structure == #
        return new(arma, carma, continuous, dt, timescale)
    end
end

@inline function (X::GaussianProcess)(
    t0::T,
    W0::T,
    Z0 = nothing;
    continuous::Bool = false,
...
function Base.setproperty!(X::GaussianProcess, s::Symbol, v::Tv) where {Tv}
    ...
    if (s == :dt)
        vnew = ARMAProcess(
            discrete.powerspectrum,
            s,
            arma.timescale;
            arma.mean,
            arma.normalize_sigma,
            discrete.Hfreq,
            discrete.Htime,
            arma.window_length,
            arma.coeff_length,
